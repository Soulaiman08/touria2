...
export function ProductDetail({ product, locale }: ProductDetailProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12" dir={isRTL ? 'rtl' : 'ltr'}>
      {/* ── Visual Gallery Column ───────────────────────────── */}
      <div className="md:col-span-6 space-y-4">
        <div className="relative aspect-[4/5] rounded-2xl overflow-hidden border bg-muted" style={{ borderColor: 'var(--border)' }}>
          <Image
            src={activeImage}
            alt={productName}
            fill
          ...
          <div className="flex gap-3 overflow-x-auto pb-1" dir="ltr">
            {product.images.map((img, idx) => (
              <button
                ...
                className={`relative w-20 h-20 rounded-lg overflow-hidden border flex-shrink-0 transition-all ${
      ...
      <div className="md:col-span-6 space-y-6">
        ...
        <div className="space-y-2 border-b pb-4" style={{ borderColor: 'var(--border)' }}>
          ...
          <h1 className="text-3xl font-extrabold" style={{ color: 'var(--foreground)' }}>{productName}</h1>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
          ...
          <button
            ...
            disabled={!hasStock}
            className="w-full sm:flex-1 btn btn-primary h-12 btn-round text-sm flex items-center justify-center gap-2"
          >
